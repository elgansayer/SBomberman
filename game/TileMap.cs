...
public partial class ActorSpawnPoint 
{
	public bool Used;
	public Vector2i Position;

	public ActorSpawnPoint(Vector2i position)
	{
		Position = position;
	}
}
...
public partial class TileMap : Godot.TileMap
{
	...
	public void SpawnRocks()
	{
		...
		List<ActorSpawnPoint> usedSpawnTiles = this.GetAllSpawnPoints().Where((tile) => tile.Used).ToList();
		...
		GD.Print(actualSoftBlocks.Count);
		GD.Print(maxRemove);
		...
		foreach (var softBlock in actualSoftBlocks)
		{
	...
	public List<ActorSpawnPoint> GetAllSpawnPoints()
	{
		List<Vector2i> allTiles = this.GetAllTiles(TileMapLayers.SpawnPoints);
		
		return allTiles.ConvertAll((Vector2i tile) =>
		{
			ActorSpawnPoint actorSpawnPoint = new ActorSpawnPoint(tile);
			return actorSpawnPoint;
		});
	}
}
