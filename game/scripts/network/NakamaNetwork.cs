...
namespace Network
{
    ...
    public partial class NakamaNetwork : Node
    {
        public delegate void OnPeerConnectedHandler();

        [Signal]
        public event OnPeerConnectedHandler OnPeerConnected;
        ...
        public override void _Ready()
        {
            ...
            this.client = new Nakama.Client(scheme, host, port, serverKey);
            client.Timeout = 10;
        ...
        public async Task<bool> Login(string email, string password, string username = null, bool create = false)
        {
            ...
            try
            {
                ...
                GD.Print("this.OnPeerConnected ", this.OnPeerConnected);
                GD.Print("Logged in to server ", this.Account.User.Id);

                OnPeerConnectedHandler handler = OnPeerConnected;
                handler?.Invoke();
            }
            catch (ApiResponseException ex)
            {
        ...
        public async Task UpdateAccount(PlayerAccount playerAccount)
        {
            ...
            await this.client.UpdateAccountAsync(this.session, playerAccount.Username, displayName: playerAccount.DisplayName, playerAccount.Avatar, playerAccount.LangTag, playerAccount.Location, playerAccount.Timezone);
            this.Account = await client.GetAccountAsync(this.session);
        }
